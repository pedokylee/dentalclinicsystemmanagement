...
export default function DentistProfile({ user, dentist, settings }) {
    const accountForm = useForm({
        name: user?.name ?? '',
        email: user?.email ?? '',
        specialization: dentist?.specialization ?? '',
        schedule_days: dentist?.schedule_days ?? ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'],
    });
    ...
    const toggleScheduleDay = (day) => {
        const days = accountForm.data.schedule_days.includes(day)
            ? accountForm.data.schedule_days.filter((item) => item !== day)
            : [...accountForm.data.schedule_days, day];

        accountForm.setData('schedule_days', days);
    };
        ...
        <div className="dcms-page">
            <div className="dcms-page-header">
                <div>
                    ...
                    <p className="dcms-page-subtitle">Manage your professional profile, availability schedule, password, and notification preferences.</p>
                </div>
            </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
                <section className="dcms-card">
                    <div className="dcms-card-body text-center">
                        ...
                        <p className="mt-2 text-sm text-[var(--dcms-text-soft)]">{dentist?.specialization || 'General Dentistry'}</p>
                ...
                <section className="dcms-card">
                    <div className="dcms-card-body grid gap-4 md:grid-cols-2">
                        ...
                        <div className="md:col-span-2">
                            <label className="dcms-label">Specialization</label>
                            <input className="dcms-input" value={accountForm.data.specialization} onChange={(event) => accountForm.setData('specialization', event.target.value)} />
                        </div>
                        <div className="md:col-span-2">
                            <label className="dcms-label">Working Days</label>
                            <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
                                {weekDays.map((day) => (
                                    <label key={day} className="flex items-center gap-3 rounded-2xl border border-[var(--dcms-border)] px-4 py-3">
                                        <input
                                            type="checkbox"
                                            checked={accountForm.data.schedule_days.includes(day)}
                                            onChange={() => toggleScheduleDay(day)}
                                            className="h-4 w-4 accent-[var(--dcms-primary)]"
                                        />
                                        <span className="text-sm font-medium text-[var(--dcms-text)]">{day}</span>
                                    </label>
                                ))}
                            </div>
                            {accountForm.errors.schedule_days && <p className="mt-1 text-sm text-red-500">{accountForm.errors.schedule_days}</p>}
                        </div>
                        <div className="md:col-span-2 flex justify-end">
                            <button
                                className="dcms-btn-primary"
                                onClick={() => accountForm.patch('/dentist/profile', { preserveScroll: true })}
                    ...
                    <div className="dcms-card-body grid gap-4">
                        ...
                        <div className="flex justify-end">
                            <button
                                ...
                                onClick={() =>
                                    passwordForm.patch('/dentist/profile/password', {
                                        preserveScroll: true,
                ...
                <section className="dcms-card">
                    <div className="dcms-card-body grid gap-6 md:grid-cols-2">
                        ...
                        <div className="md:col-span-2 flex justify-end">
                            <button
                                className="dcms-btn-primary"
                                onClick={() => preferencesForm.patch('/dentist/profile/preferences', { preserveScroll: true })}
